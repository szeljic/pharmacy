		<ul class="nav navbar-nav">
			<li><a id="home-link" href="panel.jsp">Home <span class="sr-only">(current)</span></a></li>
			        
			<li class="dropdown">
		        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">Drugs <span class="caret"></span></a>
		        <ul class="dropdown-menu" role="menu">
				    <li><a id="list-link" href="list_of_drugs.jsp">List of all drugs</a></li>
				    <li><a id="search-link" href="search_for_drugs.jsp">Search for a drug</a></li>
		        </ul>
	        </li>
	    </ul>
	    
	    <ul class="nav navbar-nav navbar-right">
	        <li><a id="login-link" href="login.jsp">Login</a></li>
	    </ul>